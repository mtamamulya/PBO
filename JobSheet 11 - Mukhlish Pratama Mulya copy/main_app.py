...
# Pengaturan Lokalisasi Format Mata Uang
try:
...
# Inisialisasi Pengelola Anggaran via Cache Streamlit
@st.cache_resource
    ...
    with st.form("form_transaksi_baru", clear_on_submit=True):
        ...
        if submitted:
            if not deskripsi:
                ...
                st.warning("Jumlah transaksi harus valid dan positif!")
            else:
...
# --- 2. TAMPILAN HALAMAN RIWAYAT & HAPUS DATA (FITUR PENUGASAN) ---
def halaman_riwayat(anggaran: AnggaranHarian):
    st.header("Detail Semua Transaksi")
    
    col_action1, col_action2 = st.columns([4, 2])
    ...
    if df_transaksi is None or df_transaksi.empty:
        st.info("Belum ada data transaksi pencatatan.")
    else:
        # Tampilkan tabel data transaksi utama
        st.dataframe(df_transaksi, use_container_width=True, hide_index=True)
        
        st.divider()
        # IMPLEMENTASI PENUGASAN: Antarmuka Penghapusan Item Transaksi
        ...
        if btn_hapus:
            # Melakukan pengecekan apakah ID yang dimasukkan tersedia di database
            if id_target in df_transaksi['ID'].values:
                # Membuat sistem konfirmasi keamanan
                st.warning(f"Apakah Anda yakin ingin menghapus Transaksi dengan ID {id_target}?")
                col_conf1, col_conf2 = st.columns([1, 5])
                with col_conf1:
                    if st.button("Ya, Konfirmasi"):
                        if anggaran.hapus_transaksi(int(id_target)):
                            st.success(f"Transaksi ID {id_target} sukses dihapus!")
                            st.cache_data.clear() # Bersihkan cache agar tabel ter-refresh otomatis
                            st.rerun()
                        else:
                            st.error("Gagal mengeksekusi penghapusan database.")
            else:
                st.error(f"ID Transaksi {id_target} tidak ditemukan dalam records.")

# --- 3. TAMPILAN HALAMAN RINGKASAN ---
def halaman_ringkasan(anggaran: AnggaranHarian):
    ...
    if not dict_per_kategori:
        ...
        try:
            ...
            st.error(f"Gagal menampilkan ringkasan visualizer: {e}")

# --- FUNGSI UTAMA NAVIGASI APLIKASI ---
def main():
    st.sidebar.title("Menu Catatan Pengeluaran")
