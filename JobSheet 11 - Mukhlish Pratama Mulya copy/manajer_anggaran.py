...
class AnggaranHarian:
    ...
    # FITUR PENUGASAN: Hapus Transaksi berdasarkan ID
    def hapus_transaksi(self, id_transaksi: int) -> bool:
        ...
        params = (id_transaksi,)
        # Jika eksekusi query tidak menghasilkan None, berarti proses berhasil
    ...
    def get_dataframe_transaksi(self, filter_tanggal: datetime.date | None = None) -> pd.DataFrame:
        # Modifikasi query agar menyertakan ID untuk mempermudah fitur hapus data di frontend
        ...
        if not df.empty:
            ...
            # Ubah format penamaan kolom agar informatif di tabel Streamlit
            df.columns = ['ID', 'Tanggal', 'Kategori', 'Deskripsi', 'jumlah_raw', 'Jumlah (Rp)']
